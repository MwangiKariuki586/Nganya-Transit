...
import { MARKER_VISUAL } from "@/lib/tracking-signal";

// ─── Signal type config ───────────────────────────────────────────────────────

/**
 * MARKER_VISUAL is the single source of truth for marker styling.
 * Imported from @/lib/tracking-signal — do not duplicate here.
 *
 * Stale markers additionally render a dashed outer ring (dashedRing: true)
 * to visually communicate "last-known location" rather than active movement.
 */
...
function TearDropPin({
  ...
  opacity?: number;
  pulse?: boolean;
  /** Renders a dashed outer ring to signal last-known / stale state */
  dashedRing?: boolean;
  ...
  return (
    ...
    >
      ...
      {/* STALE dashed outer ring — communicates "last-known location" */}
      {dashedRing && (
        <div
          aria-hidden="true"
          style={{
...
function PinFace({
  ...
  return <MatatuTopDown size={size} heading={heading} dimmed={dimmed} />;
}
...
export function NganyaMarker({
  ...
  const cfg = MARKER_VISUAL[signalType];
  ...
  // EXPIRED markers are never rendered on the live map
  if (signalType === "EXPIRED") return null;

  const label = name?.trim() ?? "";
  const showLabel = label.length > 0;

  const isStale = signalType === "STALE";

  const pinBlock = (
    <div style={{ position: "relative" }}>
      {cfg.trail && <MotionTrail heading={heading} />}
  ...
  return (
    ...
    >
      <div style={getMarkerNameChipStyle("nganya")}>{label}</div>
      {pinBlock}
    </div>
  );
}
...
export function StageMarker({ name, size = 40 }: StageMarkerProps) {
  return (
    ...
    >
      ...
      <div style={getMarkerNameChipStyle("stage")}>{name}</div>

...
import { Clock, MapPin, Map, Navigation2 } from 'lucide-react'

const ToastMessage = ({ message, show }: { message: string, show: boolean }) => {
    if (!show) return null
    return (
...
export default function SearchResultsOverlay({ isOpen, onClose, fromStage, toPlace, preference, preferredNganya, inline = false }: Props) {
    ...
    const loadResults = async (checkCloser: boolean = false) => {
        ...
        try {
            const data = await searchNganyaJourney({
                ...
                preferredNganyaId: preference === 'SPECIFIC' ? preferredNganya?.id : null,
            ...
            const nextBestEta = corridorScoped.length > 0
                ? Math.min(...corridorScoped.map((item) => item.eta_minutes).filter((eta: number) => Number.isFinite(eta)))
                : null

            if (checkCloser && bestEtaRef.current !== null && nextBestEta !== null && nextBestEta < bestEtaRef.current) {
            ...
            console.log('Analytics: ride_search_results_loaded', { count: corridorScoped.length, corridor: corridorId })
        } catch (e) {
    ...
    useEffect(() => {
        ...
        const channel = supabase.channel(`searchResultsRealtime_${corridorId}`)
            .on(
                'postgres_changes',
                {
                    event: '*',
    ...
    const getStatusInfo = (eta: number, confidence: string) => {
        if (eta <= 12 && confidence !== 'LOW') {
            return { label: 'Catchable', color: 'text-[var(--color-green)]', cta: 'Track' }
        }
        if (eta > 20 || confidence === 'LOW') {
            return { label: 'Too Far', color: 'text-[var(--color-text-tertiary)]', cta: 'Follow + Alerts' }
        }
        return { label: 'Risky', color: 'text-[var(--color-accent)]', cta: 'Track' }
    }
    ...
    const getLastSeenText = (item: any) => {
        ...
        const elapsedSeconds = Math.max(0, Math.floor((Date.now() - seenAt) / 1000))
        if (item.source === 'LIVE') {
    ...
    const preferenceLabel = useMemo(() => {
        if (preference === 'SPECIFIC') return preferredNganya ? `Specific (${preferredNganya.name})` : 'Specific'
            ...
            <div className="mb-6">
                ...
                <div className="space-y-3">
                    {items.map((r) => {
                        ...
                        return (
                            <div key={r.nganya_id} className="bg-[var(--glass-bg)] border border-[var(--glass-border)] rounded-[var(--radius-lg)] p-4 flex flex-col gap-3">
                                <div className="flex justify-between items-start">
                                    <div>
                                        <h3 className="text-h3 text-[var(--color-text-primary)]">{r.nganya_name}</h3>
                                        <p className="text-xs text-[var(--color-text-tertiary)]">
                                            {r.corridor_name} {r.tags && r.tags.length > 0 ? `- ${r.tags.join(', ')}` : ''}
                                        </p>
                                    </div>
                                    <div className="text-right">
                                        <div className="text-h2 text-[var(--color-accent)]">{r.eta_minutes} <span className="text-sm font-normal">min</span></div>
                                        <div className={`text-[10px] font-bold uppercase ${status.color}`}>{status.label}</div>
                                    </div>
                                </div>
                                ...
                                <div className="flex justify-between items-center mt-2 pt-3 border-t border-[var(--glass-border)]">
                                    ...
                                    <Button
                                        ...
                                        onClick={() => {
                                            if (status.cta === 'Track') {
                                                console.log('Analytics: result_selected_track', { nganya_id: r.nganya_id })
                                                setTrackingNganya(r)
                                                return
                                            }
                                            console.log('Analytics: follow_from_results', { nganya_id: r.nganya_id })
                                        }}
                                    >
                                        {status.cta === 'Track' ? <><Navigation2 className="w-4 h-4 mr-1" />Track</> : 'Follow + Alerts'}
                                    </Button>
                                </div>
                            </div>
                        )
    ...
    const content = (
        <div className={`flex flex-col relative ${inline ? '' : 'h-full max-h-[85vh]'}`}>
            <ToastMessage message={toastMsg} show={!!toastMsg} />
            <div className="mb-4 space-y-1">
                <div className="flex items-center gap-2 text-sm text-[var(--color-text-secondary)]">
                    <Map className="w-4 h-4 text-[var(--color-accent)]" />
                    Route: <span className="font-semibold text-[var(--color-text-primary)] truncate max-w-[170px]">{toPlace.name}</span>
                </div>
                <div className="flex items-center gap-2 text-sm text-[var(--color-text-secondary)]">
                    <MapPin className="w-4 h-4" />
                    Pickup: <span className="font-semibold text-[var(--color-text-primary)] truncate max-w-[170px]">{fromStage.name}</span>
                </div>
                <div className="text-sm text-[var(--color-text-secondary)]">
                    Preference: <span className="font-semibold text-[var(--color-text-primary)]">{preferenceLabel}</span>
                </div>
            </div>
            ...
            <div className={`${inline ? '' : 'flex-1 overflow-y-auto'} pb-6`}>
                {isLoading ? (
                    ...
                    <div className="text-center p-8 border border-dashed border-[var(--color-line)] rounded-[var(--radius-lg)]">
                        <p className="text-[var(--color-text-secondary)] mb-2">No active rides found for this corridor right now.</p>
                        <Button variant="secondary" onClick={onClose}>Adjust Search</Button>
                    </div>
                )}
            </div>

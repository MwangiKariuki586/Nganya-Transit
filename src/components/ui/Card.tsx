...
export default function Card({
    ...
    if (variant === 'compact') return <CompactCard nganya={nganya} isFollowing={isFollowing} onFollow={onFollow} className={className} />
    if (variant === 'feature') return <FeatureCard nganya={nganya} isFollowing={isFollowing} onFollow={onFollow} className={className} />
    return <StandardCard nganya={nganya} isFollowing={isFollowing} onFollow={onFollow} className={className} />
}

/* ─── Standard Card ───────────────────────────────────────── */
function StandardCard({ nganya, isFollowing, onFollow, className }: CardProps) {
    return (
        <Link
            to="/nganya/$slug"
        ...
        >
            ...
            <div className="p-4">
                ...
                <div className="flex items-center gap-2 mb-3 text-[var(--color-text-tertiary)]">
                    <span className="text-body-sm">{nganya.corridor}</span>
                </div>
    ...
    return (
        ...
        >
            ...
            <div className="relative w-14 h-14 rounded-[var(--radius-md)] overflow-hidden shrink-0">
                <img src={nganya.imageUrl} alt={nganya.name} className="w-full h-full object-cover" />
                {nganya.isLive && (

    ...
    return (
      <div className="page-container max-w-3xl py-8 md:py-10">
        <div className="rounded-[28px] border border-[var(--color-accent)]/30 bg-[var(--glass-bg-strong)] p-6 shadow-[var(--glow-accent-sm)]">
          <div className="text-tag text-[var(--color-accent)]">{statusCopy.eyebrow}</div>
          ...
          <div className="mt-5 grid gap-3 rounded-[20px] border border-[var(--glass-border)] bg-[rgba(10,10,15,0.55)] p-4 text-sm text-[var(--color-text-secondary)] md:grid-cols-2">
            <div>Route: {activeRequest.corridors?.name || 'Unknown corridor'}</div>
            <div>Status: {activeRequest.status}</div>
            <div>Name: {activeRequest.proposed_name}</div>
            <div>Submitted: {new Date(activeRequest.created_at).toLocaleString()}</div>
          </div>
          ...
          {activeRequest.review_notes ? (
            <div className="mt-5 rounded-[20px] border border-amber-500/20 bg-amber-500/8 p-4 text-sm text-[var(--color-text-secondary)]">
              <div className="font-semibold text-[var(--color-text-primary)]">Review note</div>
  ...
  return (
    <div className="page-container max-w-3xl py-8 md:py-10">
      <div className="mb-6 flex items-center gap-4">
        ...
        <div>
          <div className="text-tag text-[var(--color-accent)]">Crew registration</div>
      ...
      {entryReason === 'mapping-required' ? (
        <div className="mb-5 rounded-[24px] border border-[var(--color-accent)]/30 bg-[var(--color-accent)]/10 p-4 text-sm text-[var(--color-text-secondary)] shadow-[var(--glow-accent-sm)]">
          <div className="font-semibold text-[var(--color-text-primary)]">Register first to unlock Go Live</div>
          <div className="mt-1">
            This crew account has no linked nganya yet. Submit one for review first, then go live after approval and mapping.
          </div>
        </div>
      ) : null}
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <section className="rounded-[28px] border border-[var(--glass-border)] bg-[rgba(23,23,31,0.94)] p-5 shadow-[var(--shadow-md)] md:p-6">
          <div className="text-caption text-[var(--color-text-tertiary)]">1. Route terminal</div>
          <select
            value={corridorId}
        ...
        <section className="rounded-[28px] border border-[var(--glass-border)] bg-[rgba(23,23,31,0.94)] p-5 shadow-[var(--shadow-md)] md:p-6">
          <div className="text-caption text-[var(--color-text-tertiary)]">2. Build details</div>
          <div className="mt-3 grid gap-4 md:grid-cols-2">
            <div>
              <label className="text-body-sm text-[var(--color-text-secondary)]">Nganya name</label>
              <input
                value={proposedName}
            ...
            <div>
              <label className="text-body-sm text-[var(--color-text-secondary)]">Plate info</label>
              <input
                value={plateInput}
            ...
            <div className="md:col-span-2">
              <label className="text-body-sm text-[var(--color-text-secondary)]">SACCO (optional)</label>
              <input
                value={sacco}
        ...
        <section className="rounded-[28px] border border-[var(--glass-border)] bg-[rgba(23,23,31,0.94)] p-5 shadow-[var(--shadow-md)] md:p-6">
          <div className="text-caption text-[var(--color-text-tertiary)]">3. Photos</div>
          <label className="mt-3 flex min-h-[160px] cursor-pointer flex-col items-center justify-center rounded-[20px] border border-dashed border-[var(--glass-border-hover)] bg-[rgba(10,10,15,0.4)] px-4 py-5 text-center transition-all hover:border-[var(--color-accent)] hover:bg-[rgba(255,45,120,0.06)]">
            ...
            <div className="mt-3 text-sm font-semibold text-[var(--color-text-primary)]">Add 1 to 3 photos</div>
            <div className="mt-1 text-body-sm text-[var(--color-text-secondary)]">Camera-first. Clear front, side, or rear shots help admin verify faster.</div>
          ...
          <div className="mt-4 rounded-[20px] border border-[var(--glass-border)] bg-[rgba(10,10,15,0.4)] p-4">
            <div className="text-body-sm text-[var(--color-text-secondary)]">Or paste an image URL</div>
          ...
          {filePreviews.length > 0 ? (
            <div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-3">
              {filePreviews.map((item, index) => (
                <div key={`${item.file.name}-${index}`} className="relative overflow-hidden rounded-[18px] border border-[var(--glass-border)] bg-[rgba(10,10,15,0.55)]">
                ...
                >
                  {brokenImageUrls.includes(url) ? (
                    <div className="flex h-32 w-full flex-col items-center justify-center px-3 text-center">
                      <div className="text-body-sm text-[var(--color-text-primary)]">Image unavailable</div>
                      <div className="mt-1 text-caption text-[var(--color-text-tertiary)]">This URL could not be rendered as an image.</div>
                    </div>
                  ) : (
                    <img
                      src={url}
        ...
        <section className="rounded-[28px] border border-[var(--glass-border)] bg-[rgba(23,23,31,0.94)] p-5 shadow-[var(--shadow-md)] md:p-6">
          <div className="text-caption text-[var(--color-text-tertiary)]">4. Tags</div>
          ...
          <div className="mt-4 rounded-[20px] border border-[var(--glass-border)] bg-[rgba(10,10,15,0.4)] p-4">
            <div className="text-body-sm text-[var(--color-text-secondary)]">Add your own tag</div>
            <div className="mt-3 flex flex-col gap-3 sm:flex-row">
              <input
        ...
        <div className="sticky bottom-4 z-10 rounded-[24px] border border-[var(--glass-border)] bg-[var(--color-bg-base)]/92 p-3 backdrop-blur-xl">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <div className="text-caption text-[var(--color-text-tertiary)]">Review queue</div>
              <div className="text-body-sm text-[var(--color-text-secondary)]">Submitted builds stay private until admin approval.</div>
            </div>
            <Button
              type="submit"
              variant="primary"

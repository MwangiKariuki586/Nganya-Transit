...
export default function SpotScreen() {
  ...
  const corridors = useNganyaStore((state) => state.corridors);
  const isLoadingNganyas = useNganyaStore((state) => state.isLoadingNganyas);
  const isLoadingCorridors = useNganyaStore(
    (state) => state.isLoadingCorridors,
  );
  const fetchNganyas = useNganyaStore((state) => state.fetchNganyas);
  const fetchCorridors = useNganyaStore((state) => state.fetchCorridors);
  const postSightingAction = useSightingStore((state) => state.postSighting);
  ...
  const isLoading = isLoadingNganyas || isLoadingCorridors;
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        await Promise.all([fetchCorridors(), fetchNganyas("")]);
      } catch (error) {
    ...
    loadData();
  }, [fetchCorridors, fetchNganyas]);

  const filteredNganyas =
    searchQuery && nganyas
      ? nganyas.filter(
          (nganya) =>
            nganya.name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
            nganya.corridors?.name
              ?.toLowerCase()
              .includes(searchQuery.toLowerCase()),
        )
      : nganyas || [];

  const selectedNganyaData = nganyas?.find(
    (nganya) => nganya.id === selectedNganya,
  );
    ...
    return (
      <div className="page-container py-16 flex flex-col items-center justify-center text-center min-h-[60vh]">
        ...
        <p className="text-body text-[var(--color-text-secondary)]">
          Thanks for spotting <strong>{selectedNganyaData?.name}</strong>. The
          feed is live.
        </p>
      </div>
    );
  ...
  return (
    <div className="page-container pt-8 pb-10 md:pt-12 md:pb-16 max-w-xl mx-auto">
      ...
      <div className="flex gap-1.5 mb-8">
        {(["select", "location", "media", "confirm"] as const).map(
          (currentStep, index) => (
            <div
              key={currentStep}
              className={`h-1 flex-1 rounded-full transition-colors ${
                index <=
                ["select", "location", "media", "confirm"].indexOf(step)
                  ? "bg-[var(--color-accent)]"
                  : "bg-[var(--glass-bg)]"
              }`}
            />
          ...
          <div className="space-y-2 max-h-[50vh] overflow-y-auto">
            {filteredNganyas.map((nganya) => (
              <button
                ...
                className={`w-full flex items-center gap-3 p-3 rounded-[var(--radius-md)] border transition-all text-left cursor-pointer ${
                  selectedNganya === nganya.id
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <span className="text-sm font-semibold text-[var(--color-text-primary)] block truncate">
                    {nganya.name}
                  </span>
                  <span className="text-xs text-[var(--color-text-tertiary)]">
                    {nganya.corridors?.name || "Unknown Route"}
                  </span>
                </div>
              </button>
            ))}
          ...
          <div className="p-4 rounded-[var(--radius-md)] bg-[var(--glass-bg)] border border-[var(--glass-border)] flex items-start gap-3">
            ...
            <div className="text-sm text-[var(--color-text-primary)]">
              <div className="font-medium mb-1">
                We will use your current device location when you post.
              </div>
              <div className="text-[var(--color-text-tertiary)]">
                Choose the corridor now. On submit, MATWANA asks for live
                location permission instead of faking coordinates.
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            {corridors.map((corridor) => (
              <button
                ...
                className={`w-full flex items-center justify-between p-3 rounded-[var(--radius-md)] border transition-all cursor-pointer ${
        ...
        <div className="space-y-6">
          <label className="flex flex-col items-center justify-center gap-3 p-8 rounded-[var(--radius-lg)] border-2 border-dashed border-[var(--glass-border)] hover:border-[var(--color-accent)] bg-[var(--glass-bg)] cursor-pointer transition-colors">
            ...
            <input
              ...
              className="hidden"
              onChange={(event) =>
                setSelectedPhotoName(event.target.files?.[0]?.name || null)
              }
            />
          </label>
      ...
      {step === "confirm" && (
        <div className="space-y-6">
          <div className="p-5 rounded-[var(--radius-lg)] bg-[var(--glass-bg)] border border-[var(--glass-border)] space-y-4">
            <h3 className="text-h4 text-[var(--color-text-primary)]">
              Sighting Summary
            </h3>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-[var(--color-text-primary)]">
                  {selectedNganyaData?.name}
                </p>
                <p className="text-xs text-[var(--color-text-tertiary)]">
                  {
                    corridors.find(
                      (corridor) => corridor.id === selectedCorridor,
                    )?.name
                  }
                </p>
              </div>
            ...
            <div className="flex gap-2 flex-wrap">
              <Chip
                label={`Confidence: ${confidence.toUpperCase()}`}
                variant="route"
              />
